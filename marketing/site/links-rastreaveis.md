# 🔗 Catálogo de links rastreáveis — Juliana

> Cada lugar onde a Ju divulga usa um link diferente. O `?origem=` no fim diz ao sistema
...
- O link da captura é sempre: `.../investir.html?origem=NOME`
- Troque o `NOME` pelo canal. **Sempre minúsculo, sem espaço, sem acento** (use hífen: `lancamento-essence`).
- Se esquecer o `?origem=`, o sistema tenta adivinhar pelo referrer (Instagram, Google, bio) — mas o **jeito certo e preciso** é sempre marcar no link.

---

## Os links prontos (copia e cola)
...
5. **Em campanha de um empreendimento específico:** cria um `?origem=lancamento-X` — e mede aquele lançamento isolado.

## Por que isso é ouro 💰

No painel, a seção **"Por origem"** vai mostrar, por exemplo:
```
story    ████████████  18 leads (120 vis · 15%)
ads      ██████        9 leads  (200 vis · 4,5%)
bio      ████          6 leads  (40 vis · 15%)
```
Leitura: *"meus stories convertem 3x melhor que meu anúncio pago — vou postar mais story e revisar a campanha."* **Isso é decisão baseada em dado** — exatamente o que justifica o plano Gestão e o seu posicionamento de consultor.

> 💡 Dica de encurtador: links longos com `?origem=` ficam feios no story. Use um encurtador
> grátis (bit.ly, encurtador do próprio Insta) — o `?origem=` continua funcionando por trás.
